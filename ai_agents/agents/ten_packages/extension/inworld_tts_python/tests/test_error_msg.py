...
def test_empty_params_fatal_error():
    ...
    tester.set_test_mode_single(
        "inworld_tts_python", json.dumps(empty_params_config)
    )
...
def test_invalid_api_key_error(MockInworldTTSClient):
    """Test that an invalid API key is handled correctly with a mock."""
    print("Starting test_invalid_api_key_error with mock...")

    # The real client yields INVALID_KEY_ERROR when the service returns
    # 401; simulate that at the client boundary.
    mock_instance = MockInworldTTSClient.return_value
    mock_instance.clean = AsyncMock()

    async def mock_get(text: str, request_id: str | None = None):
        yield (
    ...
    tester.set_test_mode_single(
        "inworld_tts_python", json.dumps(invalid_key_config)
    )
    ...
    assert (
        vendor_info.get("vendor") == "inworld"
    ), f"Expected vendor 'inworld', got {vendor_info.get('vendor')}"
