...
class FashionAIExtension(AsyncExtension):
    ...
    async def on_init(self, _: AsyncTenEnv) -> None:
        logger.info(
            "FASHION_AI on_init *********************************************************"
        )
        self.stopped = False
    ...
    async def on_stop(self, _: AsyncTenEnv) -> None:
        logger.info("FASHION_AI on_stop")
        self.stopped = True
        await self.queue.put(None)

    async def on_deinit(self, _: AsyncTenEnv) -> None:
        logger.info("FASHION_AI on_deinit")

    async def on_cmd(self, ten_env: AsyncTenEnv, cmd: Cmd) -> None:
        cmd_name = cmd.get_name()
        logger.info(f"FASHION_AI on_cmd name {cmd_name}")
    ...
    async def on_data(self, _: AsyncTenEnv, data: Data) -> None:
        ...
        logger.info("FASHION_AI on data %s", input_text)
        try:
            await self.queue.put(input_text)
        except asyncio.TimeoutError:
            logger.warning(f"FASHION_AI put inputText={input_text} queue timed out")
        except Exception as e:
            logger.warning(f"FASHION_AI put inputText={input_text} queue err: {e}")
        logger.info("FASHION_AI send_inputText %s", input_text)
    ...
    async def process_input_text(self):
        while True:
            ...
            logger.info(f"async_polly_handler: loop fashion ai polly.{input_text}")
