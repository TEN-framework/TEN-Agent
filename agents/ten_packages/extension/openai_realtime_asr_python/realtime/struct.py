...
class InputAudioTranscription:
    ...
    prompt: Optional[str] = None  # Optional prompt for the transcription model
    language: Optional[str] = None  # Optional language for the transcription model
...
class ServerVADUpdateParams:
    ...
    create_response: Optional[bool] = None  # Whether to create a response based on the detected speech
    type: str = "server_vad"  # Fixed value for VAD type

@dataclass
class InputAudioNoiseReduction:
    type: Literal["near_field", "far_field"]  # Type of noise reduction, either "near_field" or "far_field"
...
# Below is example of the JSON structure for the transcription session
# {
...
class TranscriptionSession:
    id: str  # The unique identifier for the transcription session
    expires_at: int  # Expiration time of the session in seconds since the epoch (UNIX timestamp)
    input_audio_format: AudioFormats # The audio format for input
    turn_detection: ServerVADUpdateParams # Voice activity detection settings
    object: str = "realtime.transcription_session"  # Fixed value for object type
    input_audio_transcription: Optional[List[InputAudioTranscription]] = None # List of transcription settings
    input_audio_noise_reduction: Optional[InputAudioNoiseReduction] = None # Noise reduction settings
    include: Optional[List[str]] = None # List of included fields
    client_secret: Optional[str] = None # Client secret for the session



# Below is example of the JSON structure for the transcription session update params
# {
...
# }
@dataclass
class TranscriptionSessionUpdateParams:
...
@dataclass
class TranscriptionSessionCreated(ServerToClientMessage):
    session: TranscriptionSession
    type: str = EventType.TRANSCRIPTION_SESSION_CREATED
...
def parse_server_message(unparsed_string: str) -> ServerToClientMessage:
    ...
    elif data["type"] == EventType.TRANSCRIPTION_SESSION_CREATED:
        return from_dict(TranscriptionSessionCreated, data)
