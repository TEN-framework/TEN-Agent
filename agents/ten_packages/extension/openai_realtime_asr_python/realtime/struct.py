...
class ServerVADUpdateParams:
    ...
    create_response: Optional[bool] = (
        None  # Whether to create a response based on the detected speech
    )
...
class TranscriptionSession:
    ...
    input_audio_transcription: Optional[List[InputAudioTranscription]] = (
        None  # List of transcription settings
    )
    input_audio_noise_reduction: Optional[InputAudioNoiseReduction] = (
        None  # Noise reduction settings
    )
...
class TranscriptionSessionUpdateParams:
    ...
    input_audio_transcription: InputAudioTranscription = InputAudioTranscription(
        model="gpt-4o-transcribe-latest", prompt="", language="English"
    )
...
class TranscriptionSessionUpdate(ClientToServerMessage):
    session: Optional[TranscriptionSessionUpdateParams] = field(
        default=None
    )  # Assuming `TranscriptionSessionUpdateParams` is defined
